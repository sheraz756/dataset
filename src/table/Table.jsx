...
const Table = () => {
  ...
  useEffect(() => {
    ...
    // setData(response.data.Data.RoomList)
  ...
  return (
    <div>
      ...
      <div className={styles.buttonCover}>
        <div className={styles.today}><button className={styles.btnToday}>Today</button></div>
        <div className={styles.yesterday}><button className={styles.btnYesterday}>Yesterday</button></div>
      </div>
    <div className={styles.header}>
      <div><input type="search" placeholder='Search by Id' className={styles.search} /></div>
       </div>
       <div className={styles.GenrateCover}>
      <div><button className={styles.Genrate}>Genrate Report</button></div>
       </div>
        ...
        <tbody>
          {data1.map((item, index) => (
            <tr key={index}>
              ...
              <td><img src="/download.png" alt="hotelName" className={styles.HotelImg} /></td>
              ...
              <td>    <button onClick={toggleComponent } className={styles.updateBtn}>
        {showComponent ? 'Update' : 'Update'}
      </button>
    </td>
              {/* Add more table cells based on the response data structure */}
            </tr>
          ))}
